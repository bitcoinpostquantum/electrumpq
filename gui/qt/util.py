...
from electrumpq.util import FileImportFailed, FileExportFailed
from electrumpq.paymentrequest import PR_UNPAID, PR_PAID, PR_EXPIRED
...
def text_dialog(parent, title, label, ok_label, default=None, allow_multi=False, allow_subkey=False):
    ...
    l.addWidget(txt)
    if allow_subkey:
        l.addWidget(QLabel("Next subkey index:"))
        w_subkey = QLineEdit()
        w_subkey.setFixedWidth(100)
        w_subkey.setText("0")
        w_subkey.setValidator(QIntValidator(w_subkey))
        l.addWidget(w_subkey)
    l.addLayout(Buttons(CancelButton(dialog), OkButton(dialog, ok_label)))
    if dialog.exec_():
        if allow_subkey:
            return txt.toPlainText(), int(w_subkey.text())
