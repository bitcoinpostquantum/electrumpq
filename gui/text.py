...
import electrumpq
from electrumpq.util import format_satoshis, set_verbosity
from electrumpq.bitcoin import is_address, COIN, TYPE_ADDRESS
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists():
            print("Wallet not found. try 'electrumpq create'")
            exit()
    ...
    def network_dialog(self):
        ...
        if out:
            ...
            if out.get('server') or out.get('proxy'):
                proxy = electrumpq.network.deserialize_proxy(out.get('proxy')) if out.get('proxy') else proxy_config
