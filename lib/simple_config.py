...
class SimpleConfig(PrintError):
    ...
    def electrum_path(self):
        ...
        self.print_error("electrumpq directory", path)
        return path
    ...
    def get_wallet_path(self):
        ...
        if not os.path.exists(self.path):
            raise FileNotFoundError(
                _('ElectrumPQ datadir does not exist. Was it deleted while running?') + '\n' +
        ...
        old_path = os.path.join(self.path, "electrumpq.dat")
...
def read_user_config(path):
    """Parse and store the user config settings in electrumpq.conf into user_config[]."""
    if not path:
        return {}
