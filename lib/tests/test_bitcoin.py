...
from lib.bitcoin import (
    ...
    xpub_type, is_xprv, is_bip32_derivation, seed_type, EncodeBase58Check, KeyType)
...
class Test_keyImport(unittest.TestCase):
    ...
    def test_public_key_from_private_key(self):
        for priv_details in self.priv_pub_addr:
            txin_type, privkey, key_type = deserialize_privkey(priv_details['priv'])
            result = public_key_from_private_key(privkey, key_type)
            self.assertEqual(priv_details['pub'], result)
            self.assertEqual(priv_details['txin_type'], txin_type)
            self.assertEqual(priv_details['compressed'], key_type==KeyType.ECDSA_COMPRESSED)
