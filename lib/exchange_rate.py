...
class BitcoinAverage(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        return dict([(r.replace("BPQ", ""), Decimal(json[r]['last']))
    ...
    def request_history(self, ccy):
        history = self.get_csv('apiv2.bitcoinaverage.com',
                               "/indices/global/history/BPQ%s?period=alltime&format=csv" % ccy)
...
class BitcoinVenezuela(ExchangeBase):
    ...
    def get_rates(self, ccy):
        ...
        rates = [(r, json['BPQ'][r]) for r in json['BPQ']
                 if json['BPQ'][r] is not None]  # Giving NULL for LTC
        return dict(rates)
    ...
    def request_history(self, ccy):
        return self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=BPQ")[ccy +'_BPQ']
...
class Bitmarket(ExchangeBase):
    ...
    def get_rates(self, ccy):
        json = self.get_json('www.bitmarket.pl', '/json/BPQPLN/ticker.json')
...
class FxThread(ThreadJob):
    ...
    def timestamp_rate(self, timestamp):
        from electrumpq.util import timestamp_to_datetime
